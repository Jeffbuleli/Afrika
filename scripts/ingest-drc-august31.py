#!/usr/bin/env python3
"""Ingest DRC August 16-31 REPORT into seed-all.json (EN+FR), replace emdashes,
...
REPORT = ROOT / "content" / "shinta-august" / "DRC_August_31_REPORT.txt"
SEED = ROOT / "content" / "seed-all.json"
COVERS = ROOT / "public" / "covers"
OUT_META = ROOT / "content" / "shinta-august" / "seed-meta-august31.json"
...
FEATURED_TITLE_HINTS = [
    "Tshisekedi Opens National Dialogue",
    "Ebola Crisis Deepens",
    "Tanzania Backs DRC Sovereignty",
    "Ceasefire Verification Mission Deploys",
    "United States Welcomes Progress In Doha",
    "Butembo Hospital Burned",
    "AFC/M23 Captures Three Villages",
    "Kinshasa Orders Faster Disbursement",
    "African Union Warns Peace Deals",
    "Government Updates Ebola Toll",
]
...
COVER_QUERIES = [
    ("ebola|msf|who|unicef|epidemic", "Ebola virus Congo health workers PPE"),
    ("doha|switzerland|geneva|ceasefire|verification", "peace negotiation Africa meeting"),
    ("dialogue|consultation|kabund", "Felix Tshisekedi"),
    ("samia|tanzania|suluhu", "Samia Suluhu Hassan"),
    ("angola|lourenco", "Joao Lourenco"),
    ("drone|fizi", "military drone aircraft"),
    ("minembwe|ruberwa", "South Kivu hills Congo"),
    ("masisi|malemo|ufamandu", "Masisi North Kivu"),
    ("walikale|wazalendo", "Walikale North Kivu"),
    ("ituri|djugu|aru|bunia", "Bunia Ituri Congo"),
    ("finance|disbursement|macro|budget", "Kinshasa financial district"),
...
def parse_dateline_iso(body: str, fallback_day: int = 31) -> str:
            ...
            articles.append(
                {
                    ...
                    "source": "DRC_August_31_REPORT.txt",
                }
            )
            continue
        i += 1
...
def main() -> None:
    ...
    # remove prior august31 ingest if re-run
    before = len(seed)
    seed = [a for a in seed if a.get("source") != "DRC_August_31_REPORT.txt"]
    ...
    print(
        f"seed {before} -> {len(seed)} (+{len(seed_new)} august31, featured={sum(1 for a in seed_new if a['featured'])})",
        flush=True,
    )
