#!/usr/bin/env python3
"""Ingest Uganda August 16-31 REPORT into seed-all.json (EN+FR), contextual local covers."""
...
REPORT = ROOT / "content" / "shinta-august" / "UGANDA_August_31_REPORT.txt"
SEED = ROOT / "content" / "seed-all.json"
COVERS = ROOT / "public" / "covers"
OUT_META = ROOT / "content" / "shinta-august" / "seed-meta-uganda-august31.json"
MCBULELI_ENV = Path("/Users/mac/Documents/McBuleliP2P/.env")
SOURCE = "UGANDA_August_31_REPORT.txt"
...
FEATURED_HINTS = [
    "Otunnu Gains Ground In Race For Top UN Post",
    "Bobi Wine Backs Olara Otunnu",
    "Parliament Seeks Cancellation Of Entebbe Expressway",
    "Museveni Opens $125 Million Roofings Steel Plant",
    "EACOP Nears Completion",
    "State Witness Alleges Drone Plot In Besigye",
    "Tooro Palace Access Restricted",
    "Museveni Orders Official Burial For Tooro King",
    "UPDF Command Changes Signal Generational Shift",
    "Government Suspends Special Land Titles",
    "Parliament Summons Uganda Airlines",
    "Local Government Ministry Moves To Interdict 110 Staff",
]
...
COVER_POOLS: dict[str, list[str]] = {
    "museveni": [
        ...
        "/covers/Hamis_Kiggundu_presenting_to_Yoweri_Museveni.jpg",
    ],
    "kampala": [
        "/covers/KampalaSkyline.jpg",
        "/covers/KampalaSkyline2.jpg",
        "/covers/1280px-Kampala_skyline.jpg",
        "/covers/1280px-Skyline_of_Kampala.jpg",
        "/covers/1280px-Kampala-Gaddafi_Mosque.jpg",
    ],
    "victoria": [
        "/covers/1280px-View_of_Victoria_Lake_in_Uganda.jpg",
        "/covers/View_at_Lake_Victoria__Uganda_.jpg",
        "/covers/1280px-Lake_Victoria-_Kampala-Uganda.jpg",
        "/covers/1280px-Lake_Victoria-_Kampala-Uganda_1.jpg",
        "/covers/1280px-Lake_Victoria-_Kampala-Uganda_2.jpg",
    ],
    "jinja_nile": [
        "/covers/River_nile_in_Jinja.jpg",
        "/covers/1280px-Nile_Bridge_in_Jinja_City_Along_Kampala-Jinja_Highway.jpg",
        "/covers/1280px-Source_of_the_Nile__Jinja_Uganda.jpg",
        "/covers/1280px-Nile_near_Jinja_PICT0170.JPG",
    ],
    "un": [
        "/covers/1280px-Headquarters_of_the_United_Nations__New_York_City__20231001_1103_1006.jpg",
    ],
    "mining": [
        "/covers/1280px-Gold-crystals.jpg",
        "/covers/Mining_in_Kailo.jpg",
    ],
}

COVER_RULES: list[tuple[str, list[str]]] = [
    ("museveni", [r"\bmuseveni\b", r"\bnrm\b", r"\bcabinet\b", r"\blc1\b"]),
    ("un", [r"\botunnu\b", r"\bunited nations\b", r"\bsecretary-general\b", r"\bun \b"]),
    ("jinja_nile", [r"\beacop\b", r"\boil\b", r"\bpipeline\b", r"\benergy\b", r"\bpower\b", r"\binfrastructure\b", r"\brail\b", r"\broad\b"]),
    ("mining", [r"\bgold\b", r"\bmining\b", r"\bmineral\b", r"\bmanufacturing\b", r"\bindustr"]),
    ("victoria", [r"\blake\b", r"\bfisher", r"\bagricultur", r"\bhealth\b", r"\bhospital\b"]),
    ("kampala", [r"\bkampala\b", r"\bcourt\b", r"\blaw\b", r"\bpolice\b", r"\bbank\b", r"\beconomy\b"]),
]
        ...
        if line.startswith("*"):
            ...
            while i < len(lines):
                ...
                if raw.startswith("*") or re.match(
                    ...
                    break
                if raw.startswith("Uganda, Aug") or raw.startswith("REPORT ON"):
                    i += 1
                    continue
...
def openai_translate(api_key: str, base: str, model: str, item: dict) -> dict:
    system = (
        ...
        "Datelines: Kampala, August 12, 2026 - ... → Kampala, le 12 août 2026 - ..."
    )
    user = json.dumps(
        {
...
def filter_pools() -> dict[str, list[str]]:
    return {k: [p for p in paths if exists_cover(p)] for k, paths in COVER_POOLS.items()}
...
def pick_cover(
    ...
    for key, pats in COVER_RULES:
        if any(re.search(p, blob, re.I) for p in pats):
            pool_key = key
            break
    if category == "economie" and pool_key == "kampala":
        pool_key = "jinja_nile"
    if category == "securite" and pool_key == "kampala":
        pool_key = "victoria"
    pool = pools.get(pool_key) or pools.get("kampala") or ["/covers/KampalaSkyline.jpg"]
    ranked = sorted(
        pool,
...
def main() -> None:
    ...
    # keep featured short
    featured = [a for a in seed_new if a["featured"]]
    featured = sorted(featured, key=lambda a: a["publishedAt"], reverse=True)[:5]
    keep = {a["slug"] for a in featured}
    for a in seed_new:
        a["featured"] = a["slug"] in keep
        a["rank"] = 11 if a["featured"] else 53
    ...
    for a in seed:
        if a.get("featured") and a.get("country") == "UGANDA":
    ...
    print(
        f"seed {before} -> {len(seed)} (+{len(seed_new)} uganda, featured={sum(1 for a in seed_new if a['featured'])}, unique_images={len(usage)})",
        flush=True,
    )
